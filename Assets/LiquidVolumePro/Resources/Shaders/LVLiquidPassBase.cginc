 ...
 // Open GL 2.0 / WebGL support
	...
	#define WEBGL_OR_LEGACY
		...
		half4 LightingWrappedSpecular (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten) {
    	    ...
    	    c.rgb = (s.Albedo * _MainLightColor.rgb * diff + _MainLightColor.rgb * spec * diff + ambient * fresnelTerm) * atten + s.Emission;
        	c.a = s.Alpha;
        	return c;
    	}
#else
		half4 LightingWrappedSpecular (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten) {
    	    ...
    	    c.rgb = (s.Albedo * _MainLightColor.rgb * diff + ambient * fresnelTerm) * atten + s.Emission;
        	c.a = s.Alpha;
        	return c;
    	}
#endif
		...
		half4 LightingSimple (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten) {
	        half4 c;
    	    c.rgb = s.Albedo * _MainLightColor.rgb * atten + s.Emission;
        	c.a = s.Alpha;
        	return c;
    	}
		...
		half4 frag (Input IN) : SV_Target {
			...
			half atten = 1.0; //GetLightAttenuation(IN.realPos);
